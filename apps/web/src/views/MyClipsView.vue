...
const { user } = useAuth();
const { clips, total, currentPage, isLoading, error, fetchClips, deleteClip, updateClip, shareClipToChat, copyClipLink, downloadClip } = useClips();
...
<template>
  <div class="mx-auto max-w-5xl p-4">
    <div class="mb-6 flex items-center justify-between">
      <h1 class="text-2xl font-bold">My Clips</h1>
    ...
    <div v-if="isLoading && clips.length === 0" class="flex justify-center py-12">
      <div class="h-8 w-8 animate-spin rounded-full border-4 border-primary border-t-transparent" data-testid="loading-spinner" />
    </div>

    <p v-else-if="!isLoading && clips.length === 0" class="py-12 text-center text-muted-foreground" data-testid="empty-message">
      No clips yet.
    </p>
    ...
    <div v-else class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      >
        ...
        <div class="relative aspect-video bg-muted">
          ...
          <!-- Duration badge -->
          <div
            v-if="clip.durationSeconds != null"
        ...
        <div class="flex flex-1 flex-col gap-2 p-3">
          <div class="flex items-start justify-between gap-1">
            ...
            <Badge
              :variant="clip.visibility === 'private' ? 'outline' : clip.visibility === 'public' ? 'default' : 'secondary'"
          ...
          <div v-if="clip.status === 'failed'" class="mt-auto flex flex-col gap-2">
            <p class="text-xs text-destructive" data-testid="failed-message">Clip processing failed.</p>
            <Button
              size="sm"
              variant="outline"
              class="w-full"
