/* c8 ignore file -- env bootstrap: runs at startup, always mocked in tests; error path calls process.exit(1) */
import { z } from 'zod';
...
const envSchema = z
  .object({
    ...
    FRP_AGENT_PORT: z.coerce.number().optional(),
    FRP_AGENT_TOKEN: z.string().min(1).optional(),
  ...
  .refine((v) => Boolean(v.FRP_AGENT_PORT) === Boolean(v.FRP_AGENT_TOKEN), {
    message: 'FRP_AGENT_PORT and FRP_AGENT_TOKEN must be set together',
    path: ['FRP_AGENT_PORT'],
  });
