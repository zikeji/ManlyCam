...
- [ ] Task 1: Add new env vars to `apps/server/src/env.ts` (AC: #3)
  - [ ] Add `S3_ENDPOINT` (string, min 1)
  - [ ] Add `S3_BUCKET` (string, min 1)
  - [ ] Add `S3_ACCESS_KEY` (string, min 1)
  - [ ] Add `S3_SECRET_KEY` (string, min 1)
  - [ ] Add `S3_REGION` (string, min 1)
  - [ ] Add `S3_PUBLIC_BASE_URL` (string, min 1)
  - [ ] Add `S3_FORCE_PATH_STYLE` (boolean, default `true`)
  - [ ] Add `HLS_SEGMENTS_PATH` (string, default `/hls`)
  - [ ] Add `MTX_STREAM_PATH` (string, default `cam`)
  - [ ] Update `.env.example` with the new vars
  - [ ] Update test env mocks if any tests load env.ts

- [ ] Task 2: Install `@aws-sdk/client-s3` and `@aws-sdk/s3-request-presigner` (AC: #4)
  - [ ] `pnpm --filter @manlycam/server add @aws-sdk/client-s3 @aws-sdk/s3-request-presigner`

- [ ] Task 3: Create S3 client singleton at `apps/server/src/lib/s3-client.ts` (AC: #4)
  - [ ] Import from `env.ts`, construct `S3Client` with `endpoint`, `region`, `credentials`, `forcePathStyle: env.S3_FORCE_PATH_STYLE`
  - [ ] Named export only: `export const s3Client = ...`
  - [ ] Write co-located test `s3-client.test.ts`

- [ ] Task 4: Prisma schema migration for `clips` table (AC: #5)
  - [ ] Add `Clip` model to `apps/server/prisma/schema.prisma` with all columns per AC #5 (no `s3Key` field — derive from clipId)
  - [ ] Add `clips Clip[]` relation to `User` model
  - [ ] Run `pnpm prisma migrate dev --name add-clips-table`

- [ ] Task 5: Prisma schema migration for `messages` table changes (AC: #6)
  - [ ] Add `clipId` (nullable FK->clips, `onDelete: SetNull`) to `Message` model
  - [ ] Add `messageType` (String, default `'text'`) to `Message` model
  - [ ] Add `clip Clip?` relation to `Message` model
  - [ ] Add `messages Message[]` relation to `Clip` model (Prisma requires both sides)
  - [ ] Add `@@index([clipId])` to `Message` model
  - [ ] Run `pnpm prisma migrate dev --name add-message-clip-fields`

- [ ] Task 6: Update `packages/types/src/ws.ts` WsMessage types (AC: #7)
  - [ ] Add `TextChatMessage` interface (existing `ChatMessage` fields + `messageType: 'text'`)
  - [ ] Add `ClipChatMessage` interface (base fields + `messageType: 'clip'`, `clipId`, `clipThumbnailUrl`, `clipName`, `clipDurationSeconds`, `tombstone?`)
  - [ ] Retain `ChatMessage` as type alias: `export type ChatMessage = TextChatMessage | ClipChatMessage`
  - [ ] Add `clip:status-changed` to `WsMessage` union with discriminated payload on `status`
  - [ ] Add `clip:visibility-changed` to `WsMessage` union
  - [ ] Update `chat:message` payload type to `TextChatMessage | ClipChatMessage`
...
- [ ] Task 8: Update docker-compose files for shared HLS volume (AC: #2)
  - [ ] Add `hls_segments` named volume to `docs/deploy/docker-compose.yml`
  - [ ] Mount in mediamtx container read-write
  - [ ] Mount in server container read-only
  - [ ] Add S3/clipping env vars to server service environment block
  - [ ] Update traefik docker-compose similarly

- [ ] Task 9: Add `ffmpeg` to server Dockerfile (AC: #11)
  - [ ] Add `RUN apk add --no-cache ffmpeg` in the runner stage

- [ ] Task 10: Implement HLS path flush on stream offline (AC: #8)
  - [ ] In `streamService.ts` `setAdminToggle()`, when toggle is `'offline'`, call mediamtx API to flush HLS path
  - [ ] Fire-and-forget with error logging
  - [ ] Write test for the flush call

- [ ] Task 11: Write `stream_started_at` on stream online (AC: #9)
  - [ ] In `streamService.ts` `setAdminToggle()`, when toggle is `'live'`, write `stream_started_at` to `stream_config` via `streamConfig.set()`
  - [ ] Write test

- [ ] Task 12: Update `chatService.ts` for clip-aware history (AC: #10)
  - [ ] Extend `MessageRow` type with `clipId?`, `messageType`, `clip?` fields
  - [ ] Update `getHistory()` query to include `clip` relation with `select`
  - [ ] Update `toApiChatMessage()` to produce `TextChatMessage` or `ClipChatMessage` based on `messageType`
  - [ ] Implement tombstone logic: `clipId IS NULL` (cascade-nulled), clip visibility `private`, or clip `deletedAt IS NOT NULL` -> `tombstone: true`
  - [ ] `clipThumbnailUrl` = `{S3_PUBLIC_BASE_URL}/{thumbnailKey}` (import from env)
  - [ ] Omit `clipperAvatarUrl` field when null (do not serialize `null`)
  - [ ] Write comprehensive tests for text messages, clip messages, and tombstone cases

- [ ] Task 13: Run quality gates
  - [ ] `pnpm run typecheck` in `apps/server` and `apps/web`
  - [ ] `pnpm run lint` in both apps
  - [ ] `pnpm run test --coverage` in both apps
...
14. **~Mediamtx API endpoint verification~** — ACKNOWLEDGED. The endpoint `DELETE {MTX_API_URL}/v3/hlsmuxers/delete/{MTX_STREAM_PATH}` will be verified during implementation smoke testing.
...
- [ ] Update AC #3: Add `S3_FORCE_PATH_STYLE` (boolean, default `true`)
- [ ] Update AC #5: `name` VARCHAR(100), `description` VARCHAR(500), `clipper_name` VARCHAR(50)
- [ ] Update AC #5: Clarify S3 key pattern is `clips/{clipId}/video.mp4` and `clips/{clipId}/thumbnail.jpg` (no arbitrary `s3_key` field needed; derive from clipId)
- [ ] Update Task 1: Add `S3_FORCE_PATH_STYLE` env var
- [ ] Update Dev Notes: Add thumbnail fallback guidance (omit field when null, UI shows placeholder)
