...
- [ ] Task 1: Add ShadCN Dialog component (AC: #3)
  - [ ] Subtask 1.1: Install Dialog component via `pnpm dlx shadcn-vue@latest add dialog` in `apps/web/`
  - [ ] Subtask 1.2: Verify `apps/web/src/components/ui/dialog/` folder is created

- [ ] Task 2: Create notification preferences store (AC: #8, #9, #10, #12)
  - [ ] Subtask 2.1: Create `apps/web/src/composables/useNotificationPreferences.ts`
  - [ ] Subtask 2.2: Define preference shape: `{ chatMessages: boolean, mentions: boolean, streamState: boolean, flashTitlebar: boolean }`
  - [ ] Subtask 2.3: Load preferences from localStorage on init (key: `manlycam:notification-preferences`)
  - [ ] Subtask 2.4: Export reactive `preferences` ref and `updatePreference(key, value)` function
  - [ ] Subtask 2.5: Default values: all `true` (enabled)

- [ ] Task 3: Create PreferencesDialog component (AC: #3, #8)
  - [ ] Subtask 3.1: Create `apps/web/src/components/preferences/PreferencesDialog.vue`
  - [ ] Subtask 3.2: Use ShadCN Dialog with `DialogContent`, `DialogHeader`, `DialogTitle`
  - [ ] Subtask 3.3: Add Switch components for each preference toggle (Chat messages, Mentions, Stream state, Flash titlebar)
  - [ ] Subtask 3.4: Add notification permission status indicator (granted/denied/prompt)
  - [ ] Subtask 3.5: When a toggle is enabled and permission not granted, trigger permission request

- [ ] Task 4: Add "Preferences" option to Avatar dropdown (AC: #2, #3)
  - [ ] Subtask 4.1: In `BroadcastConsole.vue`, add `<button>Preferences</button>` before "Log out"
  - [ ] Subtask 4.2: Add `preferencesOpen` ref to control dialog visibility
  - [ ] Subtask 4.3: Click handler opens PreferencesDialog
  - [ ] Subtask 4.4: Import and render `PreferencesDialog` in BroadcastConsole template

- [ ] Task 5: Create useBrowserNotifications composable (AC: #1, #4, #5, #6, #7, #13)
  - [ ] Subtask 5.1: Create `apps/web/src/composables/useBrowserNotifications.ts`
  - [ ] Subtask 5.2: Export `requestPermission(): Promise<NotificationPermission>` function
  - [ ] Subtask 5.3: Export `showNotification(title: string, options: NotificationOptions): void` function
  - [ ] Subtask 5.4: Implement single-tab coordination using BroadcastChannel API (channel name: `manlycam-notifications`)
  - [ ] Subtask 5.5: On notification request, first tab to respond becomes "leader" and shows notification
  - [ ] Subtask 5.6: Only show notification if `document.hidden` is true

- [ ] Task 6: Wire chat message notifications (AC: #4)
  - [ ] Subtask 6.1: In `useChat.ts` or `ChatPanel.vue`, listen for new `chat:message` events
  - [ ] Subtask 6.2: Check if `preferences.chatMessages` is enabled
  - [ ] Subtask 6.3: Check if `document.hidden` is true
  - [ ] Subtask 6.4: Call `showNotification(senderName, { body: messagePreview })`

- [ ] Task 7: Wire mention notifications (AC: #5)
  - [ ] Subtask 7.1: In `useChat.ts` or `ChatPanel.vue`, check `mentionedUserIds` on incoming messages
  - [ ] Subtask 7.2: If current user is mentioned and `preferences.mentions` is enabled
  - [ ] Subtask 7.3: Call `showNotification('You were mentioned', { body: senderName + ': ' + messagePreview })`

- [ ] Task 8: Wire stream state notifications (AC: #6)
  - [ ] Subtask 8.1: In `useWebSocket.ts` or `WatchView.vue`, listen for `stream:state` events
  - [ ] Subtask 8.2: Check if `preferences.streamState` is enabled
  - [ ] Subtask 8.3: On state change to/from 'live', call `showNotification('Stream Update', { body: stateMessage })`

- [ ] Task 9: Wire flash titlebar preference to Story 8-2 (AC: #11)
  - [ ] Subtask 9.1: Update `useTitlebarFlash.ts` to check `preferences.flashTitlebar` before flashing
  - [ ] Subtask 9.2: Import `useNotificationPreferences` in `useTitlebarFlash.ts`
  - [ ] Subtask 9.3: In `flashTitlebar()`, return early if `preferences.value.flashTitlebar === false`

- [ ] Task 10: Update tests (AC: All)
  - [ ] Subtask 10.1: Create `useNotificationPreferences.test.ts` — test localStorage load/save
  - [ ] Subtask 10.2: Create `useBrowserNotifications.test.ts` — test single-tab coordination
  - [ ] Subtask 10.3: Create `PreferencesDialog.test.ts` — test toggle rendering and permission request
  - [ ] Subtask 10.4: Update `BroadcastConsole.test.ts` — verify Preferences button renders

- [ ] Task 11: Visual and accessibility verification (AC: All)
  - [ ] Subtask 11.1: Manual test: click Avatar, verify Preferences option appears
  - [ ] Subtask 11.2: Manual test: open Preferences, verify all toggles are present
  - [ ] Subtask 11.3: Manual test: enable Chat messages, switch tab, send message from another browser
  - [ ] Subtask 11.4: Manual test: open 2 tabs, verify only 1 notification shows
  - [ ] Subtask 11.5: Accessibility: verify Dialog has correct ARIA attributes
  - [ ] Subtask 11.6: Accessibility: verify Switch components have labels
